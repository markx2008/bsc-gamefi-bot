...
- **分紅:** 每日 00:00 自動將遊戲盈餘的 80% 同步至鏈上 StakingVault 合約。
...
### 2.2 管理員提現審核 (Withdrawal Approval)
1.  用戶提交提現申請 -> 資料庫記錄 `status: PENDING`，凍結該筆餘額。
2.  管理員透過後台（或 Bot 指令）批准申請。
3.  系統呼叫 `Vault.sol` 的提現函數發送 USDT 給用戶錢包。
4.  監聽到提現成功事件後，更新狀態為 `COMPLETED`。
