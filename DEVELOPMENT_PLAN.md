...
- [x] 建立第一版遊戲帳務資料模型與 API：猜硬幣、骰子與幸運轉盤已支援扣款、結算、玩家輸贏、遊戲金庫變動、平台收益、收益寶獎金池補貼與事件紀錄對帳。
...
- [x] 實作 7 天收益寶鎖倉 MVP：加入收益寶時從內部可用餘額鎖定本金，到期手動領回，本金與可發分紅回到非鎖倉可提款餘額。
- [x] 實作收益來源 MVP：收益寶鎖倉本金可視為配置到外部 DeFi、借貸或收益策略；外部 DeFi 賺取的收益回歸收益寶獎金池，遊戲正利潤補貼與獎金池餘額以 ledger 分錄追蹤。
- [x] 實作浮動分紅與 APY cap MVP：每期分紅不得超過單期收益上限，外部收益不足或獎金池不足時按可用獎金池發放，未分配資金留在收益寶獎金池。
