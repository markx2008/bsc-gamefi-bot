...
- **動態平衡：** 透過遊戲金庫承受玩家贏錢波動，收益寶以 7 天鎖倉、外部收益與獎金池浮動分紅控制 APY，當資金過多時利潤稀釋。
...
- [ ] 將試算模型轉成 Web 產品規格：遊戲下注使用 DB 內部餘額，不直接觸碰使用者錢包、不連外部訊息平台、不串 `/simulator` 到合約或後端寫入。
- [ ] 建立遊戲帳務資料模型與 API：扣款、結算、玩家輸贏、遊戲金庫變動、平台收益、收益寶獎金池補貼與事件紀錄需可對帳。
- [ ] 開發三款核心遊戲 Web 介面與結算邏輯：
    - **猜硬幣 (Coin Flip):** 低波動，長期莊家優勢需接近設定值。
    - **骰子比大小 (Dice):** 中波動，使用同一套內部餘額與金庫結算。
...
- [ ] 實作收益來源：外部鎖倉收益收入、遊戲正利潤補貼、初始補貼與獎金池餘額需分開記錄並可追蹤。
